...
func (h *AccountHandler) List(c *gin.Context) {
	...
	if lite {
		...
		etag := buildAccountsListETag(compact, total, page, pageSize, platform, accountType, status, search, cleanupStatus, integrationSource, true)
		if etag != "" {
			c.Header("ETag", etag)
	...
	etag := buildAccountsListETag(result, total, page, pageSize, platform, accountType, status, search, cleanupStatus, integrationSource, false)
	if etag != "" {
		c.Header("ETag", etag)
		c.Header("Vary", "If-None-Match")
		if ifNoneMatchMatched(c.GetHeader("If-None-Match"), etag) {
