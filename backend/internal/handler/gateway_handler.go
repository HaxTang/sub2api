...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			selection, err := h.gatewayService.SelectAccountWithLoadAwareness(fs.RequestContext(c.Request.Context()), apiKey.GroupID, sessionKey, reqModel, fs.FailedAccountIDs, "", int64(0)) // Gemini 不使用会话限制
			if err != nil {
	...
	for {
		...
		for {
			...
			selection, err := h.gatewayService.SelectAccountWithLoadAwareness(fs.RequestContext(c.Request.Context()), currentAPIKey.GroupID, sessionKey, reqModel, fs.FailedAccountIDs, parsedReq.MetadataUserID, subject.UserID)
			if err != nil {
				if len(fs.FailedAccountIDs) == 0 {
...
// 平台两者取并集，兼容 mixed antigravity 候选；其余平台以账号映射键为准。
