...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		selection, err := h.gatewayService.SelectAccountWithLoadAwareness(fs.RequestContext(requestCtx), apiKey.GroupID, sessionHash, reqModel, fs.FailedAccountIDs, "", int64(0))
		if err != nil {
