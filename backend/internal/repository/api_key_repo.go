	...
	m, err := r.activeQuery().
		...
		WithUser(func(q *dbent.UserQuery) {
			q.Select(
				...
				user.FieldRpmLimit,
				user.FieldCodexContinueEnabled,
				user.FieldCodexContinueMaxContinue,
				user.FieldCodexContinueRetryMax,
				user.FieldCodexContinueLowReasoningFloor,
			)
			q.WithAllowedGroups(func(gq *dbent.GroupQuery) {
				gq.Select(group.FieldID)
			})
...
func userEntityToService(u *dbent.User) *service.User {
	...
	out := &service.User{
		...
		RPMLimit:                       u.RpmLimit,
		CodexContinueEnabled:           u.CodexContinueEnabled,
		CodexContinueMaxContinue:       u.CodexContinueMaxContinue,
		CodexContinueRetryMax:          u.CodexContinueRetryMax,
		CodexContinueLowReasoningFloor: u.CodexContinueLowReasoningFloor,
