...
func (s *adminServiceImpl) CreateUser(ctx context.Context, input *CreateUserInput) (*User, error) {
	...
	user := &User{
		...
		CodexContinueEnabled: input.CodexContinueEnabled,
		Status:               StatusActive,
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	if input.CodexContinueEnabled != nil {
		user.CodexContinueEnabled = *input.CodexContinueEnabled
		fields.CodexContinue = true
	}
	if input.CodexContinueMaxContinue != nil {
		user.CodexContinueMaxContinue = *input.CodexContinueMaxContinue
		fields.CodexContinue = true
	...
	if s.authCacheInvalidator != nil {
		...
		if user.Concurrency != oldConcurrency || user.Status != oldStatus || user.Role != oldRole || user.RPMLimit != oldRPMLimit || user.CodexContinueEnabled != oldCodexContinueEnabled || user.CodexContinueMaxContinue != oldCodexContinueMaxContinue || user.CodexContinueRetryMax != oldCodexContinueRetryMax || user.CodexContinueLowReasoningFloor != oldCodexContinueLowReasoningFloor || user.RestrictPublicGroups != oldRestrictPublicGroups || !sameInt64Set(user.AllowedGroups, oldAllowedGroups) {
