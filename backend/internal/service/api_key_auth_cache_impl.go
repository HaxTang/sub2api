...
const apiKeyAuthSnapshotVersion = 24 // v24: merge Codex continuation + group long-context/model pricing + codex_models_manifest_config
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		User: APIKeyAuthUserSnapshot{
			...
			CodexContinueMaxContinue:       apiKey.User.CodexContinueMaxContinue,
			CodexContinueRetryMax:          apiKey.User.CodexContinueRetryMax,
			CodexContinueLowReasoningFloor: apiKey.User.CodexContinueLowReasoningFloor,
		},
	}
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		User: &User{
			...
			CodexContinueMaxContinue:       snapshot.User.CodexContinueMaxContinue,
			CodexContinueRetryMax:          snapshot.User.CodexContinueRetryMax,
			CodexContinueLowReasoningFloor: snapshot.User.CodexContinueLowReasoningFloor,
