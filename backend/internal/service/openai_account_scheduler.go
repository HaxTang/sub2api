...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if shouldClearStickySession(account, req.RequestedModel) || !isAccountPlatformAllowedForOpenAISchedule(account.Platform, req.Platform) || !account.IsOpenAICompatible() || !account.IsSchedulable() {
		clearBinding()
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAISelectionOrder(
	...
	buildSelectionOrder := func(pool []openAIAccountCandidateScore) []openAIAccountCandidateScore {
		...
		if !plan.includeOverflowFallback || groupTopK >= len(pool) {
			return preferOpenAIUpstreamSiteCandidates(primary, req.PreferredSiteKey)
		}
		...
		return preferOpenAIUpstreamSiteCandidates(append(primary, overflow...), req.PreferredSiteKey)
	}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if !isAccountPlatformAllowedForOpenAISchedule(account.Platform, req.Platform) || !account.IsOpenAICompatible() {
