...
func openAICompatibleAccountEligibilityFailureReasonBeforeProfit(ctx context.Context, account *Account, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) string {
	...
	if !isAccountPlatformAllowedForOpenAISchedule(account.Platform, platform) || !account.IsOpenAICompatible() {
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusionsStickyHit(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool, error) {
	...
	preferredSiteKey := s.preferredUpstreamSiteKey(ctx, groupID, sessionHash, stickyAccountID)

	// 1. 尝试粘性会话命中
	// Try sticky session hit
	if account := s.tryStickySessionHit(ctx, groupID, platform, sessionHash, requestedModel, excludedIDs, requireCompact, stickyAccountID, preferredSiteKey, requiredCapability); account != nil {
	...
	selected, compactBlocked, filterStats := s.selectBestAccount(ctx, groupID, platform, accounts, requestedModel, excludedIDs, requireCompact, preferredSiteKey, requiredCapability, preferLowUpstreamRate)
...
func (s *OpenAIGatewayService) preferredUpstreamSiteKey(ctx context.Context, groupID *int64, sessionHash string, stickyAccountID int64) string {
	if s == nil || s.settingService == nil || !s.settingService.IsUpstreamSiteAffinityEnabled(ctx) || sessionHash == "" || s.cache == nil {
		return ""
	}
	...
	if cooling, err := s.cache.IsUpstreamSiteCooling(ctx, siteKey); err == nil && cooling {
		return ""
	}
	return siteKey
}
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, platform string, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, preferredSiteKey string, requiredCapability OpenAIEndpointCapability) *Account {
	if sessionHash == "" {
		return nil
	}
	platform = NormalizeOpenAICompatiblePlatform(platform)
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, preferredSiteKey string, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, bool, openAISelectionFilterStats) {
	platform = NormalizeOpenAICompatiblePlatform(platform)
	...
	sort.SliceStable(eligible, func(i, j int) bool {
		...
		if preferredSiteKey != "" {
			aPreferred := accountUpstreamSiteKey(a) == preferredSiteKey
			bPreferred := accountUpstreamSiteKey(b) == preferredSiteKey
