<template>
  ...
  >
    <form v-if="user" id="edit-user-form" @submit.prevent="handleUpdateUser" class="space-y-5">
      ...
      <label class="flex items-start gap-3 rounded-md border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800">
        <input v-model="form.codex_continue_enabled" type="checkbox" class="mt-1 h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
        <span>
          <span class="block text-sm font-medium text-gray-900 dark:text-gray-100">{{ t('admin.users.form.codexContinueEnabled') }}</span>
...
const form = reactive({
  ...
  concurrency: 1,
  rpm_limit: 0,
  codex_continue_enabled: false,
  codex_continue_max_continue: 0,
  codex_continue_retry_max: 2,
  codex_continue_low_reasoning_floor: 150,
